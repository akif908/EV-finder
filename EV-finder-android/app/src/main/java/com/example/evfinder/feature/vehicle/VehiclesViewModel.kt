        ...
        viewModelScope.launch {
            ...
            val result = if (editing != null) {
                repository.updateVehicle(editing.id, vehicleType, registrationNo,
                    manufacturer.ifBlank { null }, model.ifBlank { null },
                    connectorType.ifBlank { null }, kwh)
            } else {
    ...
    fun delete(id: String) {
        viewModelScope.launch {
            ...
            repository.deleteVehicle(id).fold(
                ...
                onFailure = { e -> _uiState.value = _uiState.value.copy(deletingId = null, error = e.message) }
            )
        }
    }
}

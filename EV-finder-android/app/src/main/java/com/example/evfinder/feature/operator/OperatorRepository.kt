...
/** Operator-side calls: own stations, services, and station bookings. */
class OperatorRepository {
    ...
    suspend fun createStation(body: OperatorStationRequest): Result<StationDto> =
        handle { api.operatorCreateStation(body) }

    suspend fun updateStation(id: String, body: OperatorStationRequest): Result<StationDto> =
        handle { api.operatorUpdateStation(id, body) }

    suspend fun setStatus(id: String, status: String): Result<StationDto> =
        handle { api.operatorSetStationStatus(id, status) }

    suspend fun addService(stationId: String, body: OperatorServiceRequest): Result<ServiceDto> =
        handle { api.operatorAddService(stationId, body) }
    ...
    suspend fun deleteService(stationId: String, serviceId: String): Result<Unit> =
        handle { api.operatorDeleteService(stationId, serviceId) }
    ...
    private suspend fun <T> handle(call: suspend () -> Response<T>): Result<T> {
        return try {
            ...
            else if (response.isSuccessful) Result.success(Unit as T) // 204 (delete service)
